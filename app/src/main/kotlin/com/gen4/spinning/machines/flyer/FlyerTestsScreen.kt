...
private data class FlyerMotorEntry(val label: String, val id: UByte, val isLift: Boolean = false)

private val flyerMotors = listOf(
    FlyerMotorEntry("Flyer",        FlyerProtocol.MOTOR_FLYER),
    FlyerMotorEntry("Bobbin",       FlyerProtocol.MOTOR_BOBBIN),
    FlyerMotorEntry("Front Roller", FlyerProtocol.MOTOR_FRONT_ROLLER),
    FlyerMotorEntry("Back Roller",  FlyerProtocol.MOTOR_BACK_ROLLER),
    FlyerMotorEntry("Drafting",     FlyerProtocol.MOTOR_DRAFTING),
    FlyerMotorEntry("Winding",      FlyerProtocol.MOTOR_WINDING),
    FlyerMotorEntry("Lift",         FlyerProtocol.MOTOR_LIFT,       isLift = true),
    FlyerMotorEntry("Lift Left",    FlyerProtocol.MOTOR_LIFT_LEFT,  isLift = true),
    FlyerMotorEntry("Lift Right",   FlyerProtocol.MOTOR_LIFT_RIGHT, isLift = true),
)
...
fun FlyerTestsScreen(vm: FlyerViewModel) {
    ...
    Column(
        ...
        ExposedDropdownMenuBox(expanded = motorExpanded, onExpandedChange = { motorExpanded = it }) {
            OutlinedTextField(
                value = flyerMotors[motorIndex].label,
                onValueChange = {},
                readOnly = true,
                label = { Text("Motor") },
            ...
            ExposedDropdownMenu(expanded = motorExpanded, onDismissRequest = { motorExpanded = false }) {
                flyerMotors.forEachIndexed { i, m ->
...
@Composable
private fun FlyerDiagnosisResultScreen(state: FlyerDiagnosisState, onBack: () -> Unit) {
    Column(
