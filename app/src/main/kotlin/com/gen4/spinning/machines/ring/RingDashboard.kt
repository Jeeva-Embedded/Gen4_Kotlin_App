...
fun RingDashboard(
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            topBar = {
                GradientAppBar(title = "Ring Doubler", actions = {
            ...
            Box(modifier = Modifier.fillMaxSize().padding(padding)) {
                when (selectedTab) {
                    0 -> RingStatusScreen(vm = vm)
                    1 -> RingSettingsScreen(vm = vm, onNavigatePid = onNavigatePid)
                    2 -> RingTestsScreen(vm = vm)
                    3 -> RingOptionsScreen(vm = vm)
                }
            }
        }
    }
