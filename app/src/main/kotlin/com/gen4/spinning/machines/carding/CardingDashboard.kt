...
fun CardingDashboard(
    ...
    when (val cs = connectionState) {
        is ConnectionState.Lost -> {
            DisconnectedScreen(
                message = "Connection lost to ${cs.deviceName}",
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            ...
            Box(modifier = Modifier.fillMaxSize().padding(padding)) {
                when (selectedTab) {
                    0 -> CardingStatusScreen(vm = vm)
                    1 -> CardingSettingsScreen(vm = vm, onNavigatePid = onNavigatePid)
                    2 -> CardingTestsScreen(vm = vm)
                    3 -> CardingOptionsScreen(vm = vm)
                }
            }
        }
    }
