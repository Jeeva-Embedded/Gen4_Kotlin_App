...
fun DfDashboard(
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(
            topBar = {
                GradientAppBar(title = "Draw Frame", actions = {
            ...
            Box(modifier = Modifier.fillMaxSize().padding(padding)) {
                when (selectedTab) {
                    0 -> DfStatusScreen(vm = vm)
                    1 -> DfSettingsScreen(vm = vm, onNavigatePid = onNavigatePid)
                    2 -> DfTestsScreen(vm = vm)
                    3 -> DfOptionsScreen(vm = vm)
                }
            }
        }
    }
