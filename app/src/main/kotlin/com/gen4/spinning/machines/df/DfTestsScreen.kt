...
private data class DfMotorEntry(val label: String, val id: UByte)

private val dfMotors = listOf(
    DfMotorEntry("Front Roller", DfProtocol.MOTOR_FRONT_ROLLER),
    DfMotorEntry("Back Roller",  DfProtocol.MOTOR_BACK_ROLLER),
    DfMotorEntry("Creel",        DfProtocol.MOTOR_CREEL),
    DfMotorEntry("Drafting",     DfProtocol.MOTOR_DRAFTING),
)
...
fun DfTestsScreen(vm: DfViewModel) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ExposedDropdownMenuBox(expanded = motorExpanded, onExpandedChange = { motorExpanded = it }) {
            OutlinedTextField(
                value = dfMotors[motorIndex].label,
                onValueChange = {},
                readOnly = true,
                label = { Text("Motor") },
...
@Composable
private fun DfDiagnosisResultScreen(state: DfDiagnosisState, onBack: () -> Unit) {
    Column(
